...
pub const identity = .{ .classname = "weapon_gashands", .label = "Gas hands", .episode = 1, .interval = 450 };
...
pub fn heldEffect(parent: *c.refEntity_t, client: c_int) void {
    ...
    const velocity: v.Vec = .{ 0, 0, 15 };
    _ = c.CG_SmokePuff(&origin, &velocity, 4, 0.5, 0.65, 0.45, 0.18, 450, render.now(), 0, 0, c.cgs.media.smokePuffShader);
}
...
pub fn inventoryTick(player: *server.Entity) void {
    ...
    if (ps.dk3CameraActive != 0 and ps.powerups[c.PW_DK3_GASHANDS] > 0) ps.powerups[c.PW_DK3_GASHANDS] += c.level.time - c.level.previousTime;
...
pub fn loopSound(player: *server.Entity) c_int {
    const ps = &player.client[0].ps;
    return if (player.health > 0 and ps.pm_type == c.PM_NORMAL and ps.dk3CameraActive == 0 and c.DK_HasWeapon(ps, id) != 0) c.DK_SoundIndex("e1/we_gasloop.wav") else 0;
}

pub fn inventoryPrediction(controller: anytype) void {
    if (controller.ps.powerups[c.PW_DK3_GASHANDS] > 0 and controller.ps.powerups[c.PW_DK3_GASHANDS] <= controller.move.cmd.serverTime) c.DK_ExpireGasHands(controller.ps);
}
