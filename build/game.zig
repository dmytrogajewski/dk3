...
pub fn declare(b: *std.Build, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode, engine: ioq3.Engine, runtime: Runtime) void {
    ...
    for ([_]config.Product{ .qagame, .cgame, .ui }) |product| {
        const artifact = if (runtime == .legacy) ioq3.addProduct(b, target, optimize, engine.settings, product, true) else @import("replacement.zig").addProduct(b, target, optimize, product);
