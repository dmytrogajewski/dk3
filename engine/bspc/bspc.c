...
void AASOuputFile(quakefile_t *qf, char *outputpath, char *filename)
{
	...
	{
        ExtractFileBase(qf->origname, base);
        if (snprintf(filename, MAX_PATH, "%s/%s.aas", outputpath, base) >= MAX_PATH)
            Error("AAS output path exceeds %d bytes", MAX_PATH - 1);
		return;
	} //end if
	//
...
quakefile_t *GetArgumentFiles(int argc, char *argv[], int *i, char *ext)
{
	...
	{
        if (strlen(argv[(*i)+1]) >= sizeof(buf) - 8) Error("input path exceeds %d bytes", (int)sizeof(buf) - 9);
		strcpy(buf, argv[(*i)+1]);
        for (j = (int)strlen(buf) - 1; j >= 0 && buf[j] != '/' && buf[j] != '\\'; --j)
            if (buf[j] == '.') { strcpy(&buf[j+1], ext); break; }
...
int main (int argc, char **argv)
{
	...
	Log_Print("BSPC version "BSPC_VERSION" (dk3 pinned source)\n");
	...
	for (i = 1; i < argc; i++)
	{
		...
		else if (!stricmp(argv[i], "-dk3-mode"))
		{
			static const char *modes[] = {"easy", "normal", "hard", "dm", "ctf", "deathtag"};
			int mode;
			if (++i >= argc) Error("-dk3-mode requires easy, normal, hard, dm, ctf or deathtag");
			for (mode = 0; mode < 6 && strcmp(argv[i], modes[mode]); ++mode) {}
			if (mode == 6) Error("unknown -dk3-mode %s", argv[i]);
			dk3_navMode = mode;
			Log_Print("dk3 navigation mode = %s\n", modes[mode]);
		}
		...
		*/
		...
		{
			if (i + 1 >= argc) {i = 0; break;}
            if (strlen(argv[i+1]) >= sizeof(outputpath)) Error("output directory exceeds %d bytes", (int)sizeof(outputpath) - 1);
	...
	else
	{
		Log_Print("Usage:   bspc [-<switch> [-<switch> ...]]\n"
			...
			"   dk3-mode <easy|normal|hard|dm|ctf|deathtag> = filter authored dk3 brush spawns\n"
