...
static int AAS_Dk3Includes(entity_t *mapent)
{
	int flags = atoi(ValueForKey(mapent, "spawnflags"));
	const char *keys[] = {"coop", "ctf", "deathtag"};
	int enabled[] = {0, dk3_navMode == 4, dk3_navMode == 5};
	int i;
	if (dk3_navMode < 0) return true;
	for (i = 0; i < 3; ++i) {
		epair_t *pair;
		for (pair = mapent->epairs; pair && strcmp(pair->key, keys[i]); pair = pair->next) {}
		if (pair && (atoi(pair->value) != 0) != enabled[i]) return false;
	}
	if (flags & (dk3_navMode < 3 ? 0x1000 << dk3_navMode : 0x8000)) return false;
	return true;
}
...
int AAS_ValidEntity(entity_t *mapent)
{
	...
	if (!AAS_Dk3Includes(mapent)) return false;
...
// Changes Globals:		-
//===========================================================================
static void AAS_RotatingDoorAngles(entity_t *mapent, vec3_t result)
{
	int spawnflags;
	float distance;
	vec3_t movedir;
	...
	GetVectorForKey(mapent, "angles", result);
...
{
	int i;
	for (i = after + 1; i < num_entities; ++i) {
		...
		*model = atoi(reference + 1);
		...
		VectorClear(angles);
		if (!strcmp(name, "func_door_rotating")) AAS_RotatingDoorAngles(entity, angles);
		return i;
	}
	return 0;
}
...
void AAS_PositionFuncRotatingBrush(entity_t *mapent, mapbrush_t *brush)
{
	...
	vec3_t pos2;
	side_t *s;
	AAS_RotatingDoorAngles(mapent, pos2);
	for (i = 0; i < brush->numsides; i++)
	{
