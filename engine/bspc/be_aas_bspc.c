...
void BotImport_Trace(bsp_trace_t *bsptrace, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int passent, int contentmask)
{
	...
	while ((entity = AAS_NextStaticModel(entity, &model, origin, angles)) != 0) {
		trace_t brush;
		CM_TransformedBoxTrace(&brush, start, end, mins, maxs, CM_InlineModel(model),
			contentmask, origin, angles, capsule_collision);
		if (brush.allsolid || brush.fraction < result.fraction) {
			brush.startsolid |= result.startsolid;
			result = brush;
		} else if (brush.startsolid) result.startsolid = qtrue;
	}
