		...
		for (j = 0; j < area2->numfaces; j++)
		{
			...
			for (edgenum1 = 0; edgenum1 < face1->numedges; edgenum1++)
			{
				for (edgenum2 = 0; edgenum2 < face2->numedges; edgenum2++)
				{
					...
					/* A fixed inset can cross a narrow destination area and put
					   the endpoint in solid. Retain the shared-edge crossing,
					   reducing the inset until it lies inside the actual area. */
					for (inside = INSIDEUNITS_WALKEND; inside >= 0.125f; inside *= 0.5f)
					{
						VectorMA(start, inside, normal, end);
						end[2] += 0.125f;
						if (AAS_PointAreaNum(end) == area2num) break;
					}
					if (inside < 0.125f) continue;
...
// Changes Globals:		-
//===========================================================================
/* Projected ground edges can belong to overlapping, vertically separated
   areas. A small difference between their heights does not prove that a
   player can step between them. Check clearance and the supported landing
   before admitting a walking link. */
static int AAS_SupportedStep(int area1num, int area2num, vec3_t start, vec3_t end)
{
	vec3_t origin, raised, across, bottom, direction, mins, maxs;
	bsp_trace_t trace;
	int area, presence = AAS_AreaCrouch(area1num) || AAS_AreaCrouch(area2num)
		? PRESENCE_CROUCH : PRESENCE_NORMAL;
	AAS_PresenceTypeBoundingBox(presence, mins, maxs);

	/* WALKSTART points slightly into the riser. Begin the clearance probe
	   behind that edge, where the player stands before stepping up. */
	VectorSubtract(end, start, direction);
	direction[2] = 0;
	if (!VectorNormalize(direction)) return qfalse;
	VectorMA(start, -1, direction, origin);
	origin[2] += 0.25f;
	VectorCopy(origin, raised);
	raised[2] += aassettings.phys_maxstep;
	trace = AAS_Trace(origin, mins, maxs, raised, -1, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
	if (trace.startsolid || trace.fraction < 1) return qfalse;
	VectorCopy(end, across);
	across[2] = raised[2];
	trace = AAS_Trace(raised, mins, maxs, across, -1, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
	if (trace.startsolid || trace.fraction < 1) return qfalse;
	VectorCopy(across, bottom);
	bottom[2] = origin[2] - 1;
	trace = AAS_Trace(across, mins, maxs, bottom, -1, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
	if (trace.startsolid || trace.fraction == 1 ||
		trace.plane.normal[2] < aassettings.phys_maxsteepness)
		return qfalse;
	VectorCopy(trace.endpos, bottom);
	bottom[2] += 0.125f;
	area = AAS_PointAreaNum(bottom);
	if (!area) {
		vec3_t probe, points[8];
		int areas[8], count;
		/* Match BotFuzzyPointReachabilityArea's first four-unit upward
		   probe when conservative AAS expansion covers a real BSP floor. */
		VectorCopy(bottom, probe); probe[2] += 4;
		count = AAS_TraceAreas(bottom, probe, areas, points, 8);
		if (count) area = areas[0];
	}
	if (area != area2num) return qfalse;
	VectorCopy(bottom, end);
	return qtrue;
}

static int AAS_FindSupportedStep(int area1, int area2, vec3_t first1, vec3_t last1,
	vec3_t first2, vec3_t last2, vec3_t normal, vec3_t start, vec3_t end, float *height)
{
	vec3_t span1, span2, candidate1, candidate2, approach, landing;
	int sample, samples;
	VectorSubtract(last1, first1, span1);
	VectorSubtract(last2, first2, span2);
	samples = (int)(VectorLength(span2) / 16) + 1;
	if (samples > 128) samples = 128;
	for (sample = -1; sample < samples; ++sample) {
		float rise;
		if (sample < 0) { VectorCopy(start, candidate1); VectorCopy(end, candidate2); }
		else {
			float fraction = (sample + 0.5f) / samples;
			VectorMA(first1, fraction, span1, candidate1);
			VectorMA(first2, fraction, span2, candidate2);
		}
		rise = candidate2[2] - candidate1[2];
		if (rise < 0 || rise >= aassettings.phys_maxstep) continue;
		VectorMA(candidate1, INSIDEUNITS_WALKSTART, normal, approach);
		VectorMA(candidate2, INSIDEUNITS_WALKEND, normal, landing);
		if (!AAS_SupportedStep(area1, area2, approach, landing)) continue;
		VectorCopy(candidate1, start); VectorCopy(candidate2, end); *height = rise;
		return qtrue;
	}
	return qfalse;
}
...
//						Log_Write("lines no overlap: from area %d to %d\r\n", area1num, area2num);
					...
					if (groundface1->faceflags & FACE_GROUND)
					{
						/* The lowest projected edge point can be over empty space.
						   Find a supported crossing along the overlap instead of
						   discarding a usable stair elsewhere on the same edge. */
						if (dist >= 0 && dist < aassettings.phys_maxstep &&
							!AAS_FindSupportedStep(area1num, area2num, p1area1, p2area1,
								p1area2, p2area2, normal, start, end, &dist)) continue;
	...
	if (ground_foundreach)
	{
		...
		{
			VectorMA(ground_beststart, INSIDEUNITS_WALKSTART, ground_bestnormal, start);
			VectorMA(ground_bestend, INSIDEUNITS_WALKEND, ground_bestnormal, end);
			if (!AAS_SupportedStep(area1num, area2num, start, end)) return qfalse;
...
//#endif
		...
		/* Match TeleportPlayer's landing offset. Authored dk3 destinations can
		   rest exactly on an expanded AAS floor plane; the unadjusted origin
		   is classified as solid even though the runtime arrival is clear. */
		destorigin[2] += 1.0f;
		//
		area2num = AAS_PointAreaNum(destorigin);
		/* The game admits floor-overlapping anchors by tracing a bounded
		   vertical lift. Mirror that clearance search without crossing a wall. */
		for (rise = 0; rise <= 64; rise += 4)
		{
			bsp_trace_t clearance;
			VectorCopy(destorigin, end);
			end[2] += rise;
			clearance = AAS_Trace(destorigin, NULL, NULL, end, -1, CONTENTS_SOLID);
			if (clearance.startsolid || clearance.fraction < 1) break;
			trace = AAS_TraceClientBBox(end, end, PRESENCE_NORMAL, -1);
			if (!trace.startsolid)
			{
				VectorCopy(end, destorigin);
				area2num = AAS_PointAreaNum(destorigin);
				break;
			}
		}
...
void AAS_Reachability_Elevator(void)
{
	...
	{
		if (!AAS_ValueForBSPEpairKey(ent, "classname", classname, MAX_EPAIRKEY)) continue;
		/* Daikatana also authors lifts as vertical doors, operated by buttons.
		   Their physical top and bottom supply the same elevator reachability;
		   the game remains responsible for reaching and using their controls. */
		angle = 0;
		if (!strcmp(classname, "func_door")) AAS_FloatForBSPEpairKey(ent, "angle", &angle);
		if (!strcmp(classname, "func_plat") || angle == -1 || angle == -2)
		{
#ifdef REACH_DEBUG
