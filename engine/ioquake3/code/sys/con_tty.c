...
char *CON_Input( void )
{
	...
	static unsigned int pipeUsed;
	static qboolean pipeDiscard;
	int avail;
	char key;
	field_t *history;
	size_t Q_UNUSED_VAR size;
	...
	{
		int consumed;
		/* A pipe read is not a command boundary. Keep incomplete lines and
		   return complete commands individually, including a final EOF line.
		   stdin is nonblocking; bound draining an oversized line per frame. */
		for (consumed = 0; consumed < 4096; ++consumed) {
			int len = read(STDIN_FILENO, &key, 1);
			if (len < 0) return NULL;
			if (!len) stdin_active = qfalse;
			if (!len || key == '\n' || key == '\r') {
				if (pipeDiscard) {
					pipeDiscard = qfalse;
					pipeUsed = 0;
					Com_Printf("Console input line too long; discarded.\n");
					if (!len) return NULL;
					continue;
				}
				text[pipeUsed] = 0;
				if (pipeUsed) { pipeUsed = 0; return text; }
				if (!len) return NULL;
				continue;
			}
			if (!pipeDiscard) {
				if (pipeUsed == sizeof(text) - 1) pipeDiscard = qtrue;
				else text[pipeUsed++] = key;
			}
		}
	}
	return NULL;
