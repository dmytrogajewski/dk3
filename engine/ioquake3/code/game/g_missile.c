...
#ifdef DK3_GAME
    if (ent->dk.projectile) { DK_ProjectileImpact(ent, trace); return; }
#endif
	gentity_t		*other;
...
void G_RunMissile( gentity_t *ent ) {
...
#ifdef DK3_GAME
	/* Attached projectiles are advanced by their mover assembly and think timer.
	   Tracing their stationary hull again would repeatedly impact the attachment. */
	if (ent->s.eType == ET_DK3_MISSILE && ent->s.pos.trType == TR_STATIONARY) {
		G_RunThink(ent);
		return;
	}
#endif
	...
	if ( tr.fraction != 1 ) {
		...
		if ( ent->s.eType != ET_MISSILE && ent->s.eType != ET_DK3_MISSILE ) {
			return;		// exploded
		}
	}
#ifdef MISSIONPACK
