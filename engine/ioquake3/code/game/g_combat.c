...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
...
#ifdef DK3_GAME
    damage = DK_ModifyDamage(targ, attacker, damage);
    if (attacker && attacker != targ && !g_friendlyFire.integer &&
        ((DK_IsCompanion(targ) && (attacker->client || DK_IsCompanion(attacker))) ||
         (targ->client && DK_IsCompanion(attacker)))) return;
    if (targ->client && !(dflags & DAMAGE_NO_PROTECTION)) {
        if ((targ->client->ps.dk3InvincibleUntil > level.time || targ->client->ps.dk3CameraActive)) return;
        if (targ->client->ps.dk3EnvUntil > level.time && (mod == MOD_WATER || mod == MOD_SLIME || mod == MOD_LAVA)) return;
    }
#endif
...
#ifdef DK3_GAME
    if (DK_ObjectiveProtects(targ, mod)) return;
#endif
...
#ifdef DK3_GAME
        if (targ->dk.moverKind) {
            targ->health -= damage;
            if (targ->health <= 0 && targ->use) targ->use(targ, inflictor, attacker);
            return;
        }
#endif
	...
	if ( attacker->client && (client
		...
		attacker->client->ps.persistant[PERS_ATTACKEE_ARMOR] = (targ->health<<8)|(client ? client->ps.stats[STAT_ARMOR] : 0);
	}
