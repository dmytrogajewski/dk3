	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
...
#ifdef DK3_GAME
            // Authored dk3 pickups are stationary collection triggers. Their
            // generous touch bounds must not stop a nearby door or platform.
            // Dropped pickups opt into pushing through physicsObject above.
            && !check->dk.actorKind
#endif
        ) {
			continue;
...
#ifdef DK3_GAME
            {
                vec3_t contactMins, contactMaxs;
                const float *origin = check->client ? check->client->ps.origin : check->s.pos.trBase;
                VectorAdd(origin, check->r.mins, contactMins);
                VectorAdd(origin, check->r.maxs, contactMaxs);
                // Authored pickups can overlap the world. Only contact with this
                // mover requires a push; a world trace also finds unrelated solids
                // throughout a rotating brush's conservative radius bounds.
                if (!trap_EntityContact(contactMins, contactMaxs, pusher)) continue;
            }
#else
		...
		*obstacle = check;
		...
		for ( p=pushed_p-1 ; p>=pushed ; p-- ) {
            ...
            VectorCopy(p->angles, p->ent->r.currentAngles);
#endif
			if ( p->ent->client ) {
...
#ifdef DK3_GAME
void G_BeginMoverPush(void) { pushed_p = pushed; }
#endif
...
#ifdef DK3_GAME
    if (ent->dk.key && !DK_HasKey(activator, ent->dk.key)) return;
#endif
	int		total;
	int		partial;
...
#ifdef DK3_GAME
/* Restore behavior from mover semantics, never a function name or pointer in a save. */
void DK_RestoreMover(gentity_t *ent) {
    if (ent->dk.moverKind) { DK_RestoreNativeMover(ent); return; }
    if (!strcmp(ent->classname, "door_trigger")) ent->touch = Touch_DoorTrigger;
    else if (!strcmp(ent->classname, "plat_trigger")) ent->touch = Touch_PlatCenterTrigger;
    else if (!strcmp(ent->classname, "func_train")) {
        ent->reached = Reached_Train;
        ent->think = ent->nextthink > 0 ? Think_BeginMoving : NULL;
    } else if (!strcmp(ent->classname, "func_door") || !strcmp(ent->classname, "func_button") ||
               !strcmp(ent->classname, "func_plat")) {
        ent->reached = Reached_BinaryMover;
        ent->think = ent->nextthink > 0 ? ReturnToPos1 : NULL;
    }
}
#endif
