...
#ifdef DK3_GAME
    pm->ps->velocity[2] *= 1 + 0.08f * DK_Attribute(pm->ps, 3, pm->cmd.serverTime);
#endif
	PM_AddEvent( EV_JUMP );
...
#ifdef DK3_GAME
	} else if (DK_LadderMove()) {
		/* Movement and collision were handled by the shared ladder path. */
#endif
	} else if ( pm->waterlevel > 1 ) {
		// swimming
		PM_WaterMove();
