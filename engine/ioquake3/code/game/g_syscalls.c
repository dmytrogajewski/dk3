...
// dkq3 entity extension (build/ioq3_patches.zig row entity-ext-game-syscall): module code, called only after the capability query
void trap_DKQ3_LocateEntityExt( void *exts, int count, int size );
void trap_DKQ3_LocateEntityExt( void *exts, int count, int size ) {
	syscall( G_DKQ3_LOCATE_ENTITY_EXT, exts, count, size );
}

#ifdef DK3_GAME
int trap_DK3SaveWrite(int version, const char *slot, const void *data, int length) {
    return syscall(G_DK3_SAVE_WRITE, version, slot, data, length);
}
int trap_DK3SaveRead(int version, const char *slot, void *data, int capacity, qboolean previous) {
    return syscall(G_DK3_SAVE_READ, version, slot, data, capacity, previous);
}
qboolean trap_DK3HoldWorld(int version, qboolean hold) {
    return syscall(G_DK3_HOLD_WORLD, version, hold);
}
#endif
