...
jpeg_fdct_islow (DCTELEM * data, JSAMPARRAY sample_data, JDIMENSION start_col)
{
  ...
  for (ctr = 0; ctr < DCTSIZE; ctr++) {
    ...
    /* These centered coefficients may be negative.  Multiplication preserves
     * the bounded DCT scaling without a signed-left-shift overflow trap. */
    dataptr[0] = (DCTELEM) ((tmp10 + tmp11 - 8 * CENTERJSAMPLE) * (ONE << PASS1_BITS));
    dataptr[4] = (DCTELEM) ((tmp10 - tmp11) * (ONE << PASS1_BITS));
