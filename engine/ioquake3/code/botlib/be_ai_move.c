...
void BotSetBrushModelTypes(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		else if (!Q_stricmp(classname, "func_door"))
		{
			...
			modeltypes[modelnum] = angle == -1 || angle == -2 ? MODELTYPE_FUNC_PLAT : MODELTYPE_FUNC_DOOR;
		}
...
bot_moveresult_t BotTravel_Elevator(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	else
	{
		...
		if (!MoverDown(reach))
		{
			...
			/* Identify the waiting mover so native game rules can seek its
			   authored control. No switch or mover is activated in botlib. */
			for (entity = AAS_NextEntity(0); entity; entity = AAS_NextEntity(entity))
			{
				if (AAS_EntityModelNum(entity) == (reach->facenum & 0xffff))
				{
					result.blocked = qtrue;
					result.blockentity = entity;
					break;
				}
			}
			return result;
