...
/* dk3 room response: independent damped delay lines, mixed before music/voice
   streams. Delays follow the actual sample rate and reset at world boundaries. */
static void S_PaintEnvironment(int count) {
    static float delays[4][32768][2], lowpass[4][2];
    static unsigned int serial;
    static int cursor, rate;
    ...
    for (i = 0; i < count; ++i, cursor = (cursor + 1) & 32767) {
        ...
        float output[2] = {0, 0};
        for (line = 0; env->style && line < 4; ++line) {
            int at = (cursor - lengths[line]) & 32767;
        ...
        paintbuffer[i].right = s_muted->integer ? 0 : (int)Com_Clamp(-1073741824, 1073741824, input[1] + output[1] * env->reverb);
    }
}
...
//Com_Printf ("%i to %i\n", s_paintedtime, endtime);
	while ( s_paintedtime < endtime ) {
		...
		S_PaintEnvironment(end - s_paintedtime);
