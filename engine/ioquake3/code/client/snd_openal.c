...
{
    float distance = 0.0f;
    const entitySoundParams_t *params = S_EntitySoundParams(chksrc->entity);
    if (params) {
        float gain;
        distance = chksrc->local ? 0 : Distance(origin, lastListenerOrigin);
        gain = chksrc->curGain * params->volume * Com_Clamp(0, 1,
            1 - (distance - params->minimum) / (params->maximum - params->minimum));
        qalSourcef(chksrc->alSource, AL_ROLLOFF_FACTOR, 0);
        if (params->flags & 1) {
            qalSourcei(chksrc->alSource, AL_SOURCE_RELATIVE, AL_TRUE);
            qalSourcefv(chksrc->alSource, AL_POSITION, vec3_origin);
            qalSourcefv(chksrc->alSource, AL_VELOCITY, vec3_origin);
        }
        chksrc->scaleGain = gain; S_AL_Gain(chksrc->alSource, gain);
        return;
    }
...
static void S_AL_StartSound( vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx )
{
	...
	else
	{
		...
		if(S_AL_HearingThroughEntity(entnum) && !S_EntitySoundParams(entnum))
		{
	...
	for(i = 0; i < srcCount; i++)
	{
		...
		if(curSource->isLooping)
		{
			...
			if(sent->loopAddedThisFrame)
			{
                ...
                if (S_EntitySoundParams(entityNum)) S_AL_ScaleGain(curSource, curSource->loopSpeakerPos);
        ...
        if (curSource->isTracking && (!state || S_EntitySoundParams(entityNum))) {
            if (!state) qalSourcefv(curSource->alSource, AL_POSITION, entityList[entityNum].origin);
...
static ALuint environmentEffect, environmentSlot;
static LPALGENEFFECTS environmentGenEffects;
...
static LPALAUXILIARYEFFECTSLOTF environmentSlotf;
static unsigned int environmentSerial;

static void S_AL_EnvironmentShutdown(void) {
    if (environmentSlot) environmentDeleteSlots(1, &environmentSlot);
    if (environmentEffect) environmentDeleteEffects(1, &environmentEffect);
    environmentSlot = environmentEffect = 0;
}

static qboolean S_AL_EnvironmentInit(void) {
    if (!qalcIsExtensionPresent(alDevice, "ALC_EXT_EFX")) return qfalse;
#define ENV_FUNCTION(member, type, name) \
    member = (type)qalGetProcAddress(name); if (!member) return qfalse
    ENV_FUNCTION(environmentGenEffects, LPALGENEFFECTS, "alGenEffects");
    ...
    ENV_FUNCTION(environmentSlotf, LPALAUXILIARYEFFECTSLOTF, "alAuxiliaryEffectSlotf");
#undef ENV_FUNCTION
    qalGetError();
    environmentGenEffects(1, &environmentEffect);
    environmentGenSlots(1, &environmentSlot);
    environmentEffecti(environmentEffect, AL_EFFECT_TYPE, AL_EFFECT_REVERB);
    if (qalGetError() != AL_NO_ERROR) { S_AL_EnvironmentShutdown(); return qfalse; }
    environmentSerial = S_Environment()->serial - 1;
    return qtrue;
}

static void S_AL_EnvironmentUpdate(void) {
    const soundEnvironment_t *env = S_Environment();
    const float decay[5] = {1, 0.7f, 2.4f, 4, 1.5f};
    int i;
    ...
    for (i = 0; i < srcCount; ++i) {
        src_t *source = &srcList[i];
        qboolean effects = source->isActive && !source->isStream && !source->isLocked;
        qalSource3i(source->alSource, AL_AUXILIARY_SEND_FILTER,
                    effects && env->style ? environmentSlot : AL_EFFECTSLOT_NULL, 0, AL_FILTER_NULL);
        if (effects) S_AL_Gain(source->alSource, source->scaleGain * env->gain);
    }
}
...
qboolean S_AL_Init( soundInterface_t *si )
{
    ...
    if (!S_AL_EnvironmentInit()) {
        Com_Printf("OpenAL environmental effects unavailable; using software sound.\n");
