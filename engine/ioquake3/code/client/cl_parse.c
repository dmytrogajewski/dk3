...
void CL_DeltaEntity (msg_t *msg, clSnapshot_t *frame, int newnum, entityState_t *old, 
	...
	// dkq3 entity extension: the extension state rides a ring parallel to parseEntities, at the same index, and a snapshot says
	// itself whether its entities carry blocks (DKQ3_SNAPFLAG_EXT), so a snapshot that arrives before the gamestate parses
	// right. A stock snapshot leaves a zero row, which is then the old extension of any later delta from that entity.
	Com_Memset( &cl.parseEntityExts[cl.parseEntitiesNum & (MAX_PARSE_ENTITIES-1)], 0, sizeof( dkq3EntityExt_t ) );
	if ( frame->snapFlags & DKQ3_SNAPFLAG_EXT ) {
		// the old one is the ring row beside old when old is a parsed entity, else (a baseline, never received) zero
		static dkq3EntityExt_t nullExt;
		dkq3EntityExt_t *ext = &cl.parseEntityExts[cl.parseEntitiesNum & (MAX_PARSE_ENTITIES-1)];
		dkq3EntityExt_t *oldExt = &nullExt;
		if ( (uintptr_t)old >= (uintptr_t)cl.parseEntities && (uintptr_t)old < (uintptr_t)( cl.parseEntities + MAX_PARSE_ENTITIES ) ) {
			oldExt = &cl.parseEntityExts[old - cl.parseEntities];
		}
		if ( unchanged ) {
			*state = *old;
			*ext = *oldExt;
		} else {
			MSG_ReadDeltaEntityDkq3( msg, old, state, oldExt, ext, newnum );
		}
	} else
	if ( unchanged ) {
		*state = *old;
...
void CL_ParseGamestate( msg_t *msg ) {
	...
	clc.dkq3_ext = qfalse;	// dkq3 entity extension: only this gamestate's own marker can raise it
	while ( 1 ) {
		...
		} else if ( cmd == svc_dkq3_ext ) {
			// dkq3 entity extension: the server enabled it from this client's own userinfo key, so the version can differ
			// only on a demo recorded by a client of another version
			i = MSG_ReadByte( msg );
			if ( i != DKQ3_ENTITY_EXT_VERSION ) {
				Com_Error( ERR_DROP, "dkq3 entity extension v%d, this client has v%d", i, DKQ3_ENTITY_EXT_VERSION );
			}
			clc.dkq3_ext = qtrue;
			Com_Printf( "dkq3 entity extension v%d negotiated\n", i );
		} else {
