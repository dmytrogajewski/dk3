...
usercmd_t CL_CreateCmd( void ) {
	...
	if ( cl_debugMove->integer ) {
		...
		if ( cl_debugMove->integer == 3 ) {
			static usercmd_t previous;
			if (cmd.forwardmove != previous.forwardmove || cmd.rightmove != previous.rightmove ||
				cmd.upmove != previous.upmove || cmd.buttons != previous.buttons) {
				Com_Printf("dk3 input %d axes %d %d %d buttons %d ground %d flags %d velocity %.0f %.0f %.0f\n",
					cmd.serverTime, cmd.forwardmove, cmd.rightmove, cmd.upmove, cmd.buttons,
					cl.snap.ps.groundEntityNum, cl.snap.ps.pm_flags,
					cl.snap.ps.velocity[0], cl.snap.ps.velocity[1], cl.snap.ps.velocity[2]);
				previous = cmd;
			}
		}
	}
...
============
*/
/* Set only the local input orientation for reproducible gameplay scenarios.
   Movement, firing and collision still pass through ordinary user commands. */
static void CL_Dk3Look_f( void ) {
	double yaw, pitch;
	char *end;
	if (Cmd_Argc() != 3 || clc.state != CA_ACTIVE || !cl.snap.valid) {
		Com_Printf("dk3_look <yaw> <pitch>: requires an active game\n");
		return;
	}
	yaw = strtod(Cmd_Argv(1), &end);
	if (!*Cmd_Argv(1) || *end || !(yaw >= -360000 && yaw <= 360000)) {
		Com_Printf("dk3_look: invalid yaw\n");
		return;
	}
	pitch = strtod(Cmd_Argv(2), &end);
	if (!*Cmd_Argv(2) || *end || !(pitch >= -89 && pitch <= 89)) {
		Com_Printf("dk3_look: pitch must be between -89 and 89\n");
		return;
	}
	cl.viewangles[YAW] = AngleNormalize180(yaw) - SHORT2ANGLE(cl.snap.ps.delta_angles[YAW]);
	cl.viewangles[PITCH] = pitch - SHORT2ANGLE(cl.snap.ps.delta_angles[PITCH]);
}

void CL_InitInput( void ) {
	Cmd_AddCommand("dk3_look", CL_Dk3Look_f);
