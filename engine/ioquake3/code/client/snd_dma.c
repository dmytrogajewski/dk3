...
/* Authored dk3 attenuation uses the same stereo basis as the software mixer. */
static void S_SpatializeEntity(vec3_t origin, int entity, int master, int *left, int *right) {
    const entitySoundParams_t *params = S_EntitySoundParams(entity);
    vec3_t direction;
    float distance, gain, pan;
    if (!params) { S_SpatializeOrigin(origin, master, left, right); return; }
    VectorSubtract(origin, listener_origin, direction); distance = VectorNormalize(direction);
    gain = params->volume * Com_Clamp(0, 1, 1 - (distance - params->minimum) / (params->maximum - params->minimum));
    if ((params->flags & 1) || dma.channels == 1) *left = *right = (int)(master * gain);
    else {
        pan = Com_Clamp(-1, 1, -DotProduct(direction, listener_axis[1]));
        *left = (int)(master * gain * 0.5f * (1 - pan));
        *right = (int)(master * gain * 0.5f * (1 + pan));
    }
}
...
void S_AddLoopSounds (void) {
	...
	for ( i = 0 ; i < MAX_GENTITIES ; i++) {
		...
		if (loop->kill) {
			S_SpatializeEntity( loop->origin, i, 127, &left_total, &right_total);			// 3d
		} else {
			S_SpatializeEntity( loop->origin, i, 90,  &left_total, &right_total);			// sphere
		}
		...
		for (j=(i+1); j< MAX_GENTITIES ; j++) {
			...
			if (loop2->kill) {
				S_SpatializeEntity( loop2->origin, j, 127, &left, &right);				// 3d
			} else {
				S_SpatializeEntity( loop2->origin, j, 90,  &left, &right);				// sphere
			}
...
void S_Base_Respatialize( int entityNum, const vec3_t head, vec3_t axis[3], int inwater ) {
	...
	for ( i = 0 ; i < MAX_CHANNELS ; i++, ch++ ) {
		...
		if (ch->fullVolume) {
			const entitySoundParams_t *params = S_EntitySoundParams(ch->entnum);
            ch->leftvol = ch->rightvol = (int)(ch->master_vol * (params ? params->volume : 1));
		} else {
			if (ch->fixed_origin) {
			...
			S_SpatializeEntity (origin, ch->entnum, ch->master_vol, &ch->leftvol, &ch->rightvol);
		}
	}
...
static void S_OpenBackgroundStream( const char *filename ) {
	...
	/* RawSamples resamples mono/stereo 8/16-bit PCM to the device rate.
	   Supplied music need not have Quake III's original 22 kHz sample rate. */
	if (s_backgroundStream->info.rate <= 0 ||
		(s_backgroundStream->info.channels != 1 && s_backgroundStream->info.channels != 2) ||
		(s_backgroundStream->info.width != 1 && s_backgroundStream->info.width != 2)) {
		Com_Printf(S_COLOR_YELLOW "WARNING: unsupported music format in %s: %d Hz, %d channels, %d-byte samples\n",
			filename, s_backgroundStream->info.rate, s_backgroundStream->info.channels, s_backgroundStream->info.width);
		S_CodecCloseStream(s_backgroundStream);
		s_backgroundStream = NULL;
	}
}
