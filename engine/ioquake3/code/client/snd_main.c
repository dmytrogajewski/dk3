...
static entitySoundParams_t entitySoundParams[MAX_GENTITIES];
...
void S_SetEnvironment(int style, float reverb, float gain) {
    ...
    if (soundEnvironment.style != style || soundEnvironment.reverb != reverb || soundEnvironment.gain != gain) {
        soundEnvironment.style = style; soundEnvironment.reverb = reverb; soundEnvironment.gain = gain;
        ++soundEnvironment.serial;
    }
}

const soundEnvironment_t *S_Environment(void) { return &soundEnvironment; }

void S_SetEntitySoundParams(int entityNum, float volume, float minimum, float maximum, int flags) {
    entitySoundParams_t *params;
    if (entityNum < 0 || entityNum >= MAX_GENTITIES || Q_isnan(volume) || Q_isnan(minimum) || Q_isnan(maximum) ||
        volume < 0 || volume > 1 || minimum < 0 || maximum < 0 || maximum > 65536 ||
        (maximum && minimum >= maximum) || (flags & ~1))
        Com_Error(ERR_DROP, "dk3 sound interface v1: invalid entity parameters");
    params = &entitySoundParams[entityNum];
    params->volume = volume; params->minimum = minimum; params->maximum = maximum; params->flags = flags;
}

const entitySoundParams_t *S_EntitySoundParams(int entityNum) {
    return entityNum >= 0 && entityNum < MAX_GENTITIES && entitySoundParams[entityNum].maximum > 0 ?
        &entitySoundParams[entityNum] : NULL;
}
...
{
    memset(entitySoundParams, 0, sizeof(entitySoundParams));
    S_SetEnvironment(0, 0, 1);
    ++soundEnvironment.serial;
