...
void CL_Record_f( void ) {
  ...
  // sync 0 doesn't prevent recording, so not forcing it off .. everyone does g_sync 1 ; record ; g_sync 0 ..
	...
	// dkq3 entity extension: the marker this session's gamestate carried, so the demo replays only on a dkq3 client; the
	// baseline loop below then writes nothing, because an extended session received no baselines
	if ( clc.dkq3_ext ) {
		MSG_WriteByte (&buf, svc_dkq3_ext);
		MSG_WriteByte (&buf, DKQ3_ENTITY_EXT_VERSION);
		Com_Printf( "dkq3 entity extension recorded: this demo needs a dkq3 client\n" );
	}
	// baselines
...
#ifdef STANDALONE
        if (!*strver || atoi(strver) != com_protocol->integer)
            Com_Error(ERR_DROP, "dk3 protocol mismatch: server %s, client %d. Install matching dk3 builds.",
                      *strver ? strver : "unspecified", com_protocol->integer);
#endif
...
void CL_Init( void ) {
	...
	// dkq3 entity extension: the userinfo key that asks an extended server for this client's version; 0 keeps the stock
	// stream. A nonzero value BELOW this version is the archived choice of an older build (its fooconfig.cfg `seta` runs
	// before CL_Init) and is raised, so an upgrade never falls back to the stock stream unasked; any other value is sent as
	// it is, and a server of another version answers it with the stock stream (the exact-match rule, SV_DirectConnect).
	{
		cvar_t *dkq3Ext = Cvar_Get( DKQ3_ENTITY_EXT_USERINFO_KEY, va( "%d", DKQ3_ENTITY_EXT_VERSION ), CVAR_USERINFO | CVAR_ARCHIVE );
		if ( dkq3Ext->integer > 0 && dkq3Ext->integer < DKQ3_ENTITY_EXT_VERSION ) {
			Com_Printf( "%s %d is an older dkq3 entity extension: this client asks for v%d\n", DKQ3_ENTITY_EXT_USERINFO_KEY, dkq3Ext->integer, DKQ3_ENTITY_EXT_VERSION );
			Cvar_Set( DKQ3_ENTITY_EXT_USERINFO_KEY, va( "%d", DKQ3_ENTITY_EXT_VERSION ) );
		}
	}
