...
qboolean SV_DK3HoldWorld(int version, qboolean hold) {
	int i, localClients = 0;
	if (version != 1) return qfalse;
	if (hold) {
		for (i = 0; i < sv_maxclients->integer; ++i) {
			client_t *client = &svs.clients[i];
			if (client->state < CS_CONNECTED) continue;
			if (client->netchan.remoteAddress.type != NA_LOOPBACK) return qfalse;
			++localClients;
		}
		if (localClients != 1) return qfalse;
	}
	sv.dk3WorldHeld = hold;
	sv.dk3ResumeFrame = !hold;
	sv.timeResidual = 0;
	return qtrue;
}
...
void SV_Frame( int msec ) {
	...
	/* A local restore must deliver its snapshot before advancing saved deadlines.
	   Reliable commands and snapshots continue while simulation/input are held. */
	if (sv.dk3WorldHeld) {
		sv.timeResidual = 0;
	...
	if (sv.dk3ResumeFrame) {
		if (msec > frameMsec) msec = frameMsec;
		sv.dk3ResumeFrame = qfalse;
	}
