...
===============
*/
static int SV_SnapshotCapacity( void ) {
#ifdef STANDALONE
	// One shared history ring, not one worst-case world per client per packet.
	// Expired deltas already fall back to complete snapshots in sv_snapshot.c.
	int capacity = sv_maxclients->integer * PACKET_BACKUP * 128;
	int worldHistory = MAX_SNAPSHOT_ENTITIES * PACKET_BACKUP;
	return capacity > worldHistory ? capacity : worldHistory;
#else
	return sv_maxclients->integer * (com_dedicated->integer ? PACKET_BACKUP : 4) * MAX_SNAPSHOT_ENTITIES;
#endif
}
...
static void SV_Startup( void ) {
	...
	svs.numSnapshotEntities = SV_SnapshotCapacity();
...
void SV_SpawnServer( char *server, qboolean killBots ) {
	...
	svs.snapshotEntityExts = Hunk_Alloc( sizeof(dkq3EntityExt_t)*svs.numSnapshotEntities, h_high );	// dkq3 entity extension ring
...
void SV_Init (void)
{
	...
	Cvar_Get( "sv_dkq3_ext", "1", CVAR_ROM );	// dkq3 entity extension: offered to clients that ask for it; never CVAR_SERVERINFO, so a stock client sees no new key
