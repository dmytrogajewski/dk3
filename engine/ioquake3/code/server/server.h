...
typedef struct {
	...
	int				gentitySize;
	dkq3EntityExt_t	*gentityExts;		// dkq3 entity extension: the game module's rows (G_DKQ3_LOCATE_ENTITY_EXT), else NULL
	int				gentityExtCount;
...
typedef struct client_s {
	...
	int				challenge;
	int				dkq3_ext;			// dkq3 entity extension negotiated at connect (SV_DirectConnect), never changed afterwards
...
typedef struct {
	...
	dkq3EntityExt_t	*snapshotEntityExts;	// [numSnapshotEntities]: the dkq3 entity extension state beside each snapshotEntities entry
...
qboolean SV_DK3HoldWorld(int version, qboolean hold);
...
// dkq3 entity extension (sv_game.c): the game module's extension rows, and entity num's row or a zero row
