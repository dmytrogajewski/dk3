...
dkq3 entity extension: the game module's array of dkq3EntityExt_t, one row per entity, indexed like its gentities
(G_DKQ3_LOCATE_ENTITY_EXT). The row size must be the engine's, so a module built against another version is refused.
===============
*/
void SV_LocateEntityExt( void *exts, int count, int size ) {
	if ( size != sizeof( dkq3EntityExt_t ) ) {
		Com_Error( ERR_DROP, "SV_LocateEntityExt: extension struct size %d, engine %d", size, (int)sizeof( dkq3EntityExt_t ) );
	}
...
The extension row of entity num, or a zero row when the game module located none (a stock module) or num is past its count.
===============
*/
...
intptr_t SV_GameSystemCalls( intptr_t *args ) {
    ...
    case G_DK3_SAVE_WRITE:
        return FS_DK3SaveWrite(args[1], VMA(2), VMA(3), args[4]);
    case G_DK3_SAVE_READ:
	...
	case G_DKQ3_LOCATE_ENTITY_EXT:
		SV_LocateEntityExt( VMA(1), args[2], args[3] );
		return 0;
