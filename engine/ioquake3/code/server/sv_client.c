...
#endif
	{
		if(version != com_protocol->integer)
		{
			NET_OutOfBandPrint(NS_SERVER, from, "print\nServer uses protocol version %i "
					   "(yours is %i). Install matching dk3 client and server builds.\n", com_protocol->integer, version);
...
gotnewcl:	
	...
	*newcl = temp;
	...
	// dkq3 entity extension: latched exactly once, here, from the connect packet's userinfo, so a userinfo command mid-level
	// cannot switch the stream format under a parser the gamestate already set. A stock client sends no key and stays 0.
	newcl->dkq3_ext = Cvar_VariableIntegerValue( "sv_dkq3_ext" ) && atoi( Info_ValueForKey( userinfo, DKQ3_ENTITY_EXT_USERINFO_KEY ) ) == DKQ3_ENTITY_EXT_VERSION;
	if ( newcl->dkq3_ext ) {
		Com_DPrintf( "dkq3 entity extension v%d for client %i\n", DKQ3_ENTITY_EXT_VERSION, (int)( newcl - svs.clients ) );
	}
...
static void SV_SendClientGameState( client_t *client ) {
    ...
    /* New entities use zero baselines in the standalone dk3 protocol. The reviewed
       optional extension stream retains only its format marker here. */
    if (client->dkq3_ext) {
        MSG_WriteByte(&msg, svc_dkq3_ext);
        MSG_WriteByte(&msg, DKQ3_ENTITY_EXT_VERSION);
    }
...
void SV_ClientThink (client_t *cl, usercmd_t *cmd) {
	...
	if ( cl->state != CS_ACTIVE || sv.dk3WorldHeld ) {
