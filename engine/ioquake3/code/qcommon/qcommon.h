...
#define MAX_MSGLEN (256 * 1024) // dk3 reassembled message bound
...
// new commands, supported only by ioquake3 protocol but not legacy
	...
	svc_voipOpus,      //
	svc_dkq3_ext,      // dkq3 entity extension: the gamestate marker of a client that asked for it (row entity-ext-qcommon)
};

// The dkq3 entity field extension (owner decision D11, build/ioq3_patches.zig row entity-ext-qcommon): Gold's per-entity fields
// that entityState_t cannot hold, in a struct of their own that is never part of entityState_t. A client that sends the userinfo
// key below with exactly this version gets svc_dkq3_ext + one version byte in its gamestate in place of the baselines, and one
// extension block after every stock entity delta (MSG_WriteDeltaEntityDkq3); a stock peer never asks and never receives a byte
// the unpatched engine would not send. The module twin of the struct is src/game/shared/dk_entity_ext.h. Version 2 replaces
// version 1 (version 1's members, then Gold's render_scale); peers of different versions exchange the stock stream.
#define DKQ3_ENTITY_EXT_VERSION 2
#define DKQ3_ENTITY_EXT_USERINFO_KEY "cl_dkq3_ext"
// A free snapFlags bit (q_shared.h uses 1, 2 and 4) that marks a snapshot whose entities carry extension blocks, so the
// client's parser keys on the snapshot itself: the server sends SNAPFLAG_NOT_ACTIVE snapshots to a CS_CONNECTED client
// before its gamestate, when the marker has not set the session latch yet. Never set for a client that did not ask.
#define DKQ3_SNAPFLAG_EXT 0x80

// Nineteen 32-bit members and no padding: msg.c walks them by offset (dkq3EntityExtFields).
typedef struct dkq3EntityExt_s {
	int renderfx;       // Gold RF_* flags
	int effects;        // Gold EF_* flags
	int effects2;       // Gold EF2_* flags
	int modelindex2;    // the held weapon in the low 16 bits, Gold's owner tag in the high 16
	int modelindex3;    // the muzzle flash model
	int modelindex4;    // the CTF flag model
	int skinnum;
	float alpha;        // with RF_TRANSLUCENT
	vec3_t oldOrigin;   // the beam far end
	vec3_t color;
	int event;          // Gold's entity event; EV_ENTITY_FORCEMOVE means "do not lerp"
	int loopVolume;     // the looping sound's volume x 1000
	vec3_t renderScale; // version 2: Gold's s.render_scale -- a model's per-axis scale, a spotlight's far point
} dkq3EntityExt_t;
...
/* dk3 save service v1: private loose saves, durable replacement and recovery copy. */
