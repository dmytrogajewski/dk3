...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder, qboolean fixedFunction)
{
	...
	// The drawable is what OpenGL renders into, and it is not always the size that was asked for: a window manager
	// that will not change the video mode gives a fullscreen window the desktop's size, and the viewport then covers
	// a corner of it. Take the size from the drawable itself.
	{
		int drawableWidth = 0, drawableHeight = 0;
		SDL_GL_GetDrawableSize( SDL_window, &drawableWidth, &drawableHeight );
		if ( drawableWidth > 0 && drawableHeight > 0 )
		{
			if ( drawableWidth != glConfig.vidWidth || drawableHeight != glConfig.vidHeight )
			{
				ri.Printf( PRINT_ALL, "Drawable is %d %d, not the %d %d asked for\n",
						drawableWidth, drawableHeight, glConfig.vidWidth, glConfig.vidHeight );
			}
		...
		ri.Printf( PRINT_ALL, "GL drawable: %d %d\n", glConfig.vidWidth, glConfig.vidHeight );
	}
...
success:
	...
	/* dk3 keeps brightness in the rendered image. A compositor may accept SDL's
	 * gamma calls without applying the ramp; GL1 would then halve fullscreen
	 * lighting for overbright and never receive the display-side compensation.
	 * Use the existing software texture-gamma path in both renderers instead.
	 * Changing r_gamma on this path requires a video restart. */
