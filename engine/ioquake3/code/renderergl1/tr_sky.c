...
void R_BuildCloudData( shaderCommands_t *input )
{
	...
	/* Every stage uses the first pass's indices and the same base coordinates.
	   Extra copies were unreferenced and overflowed the vertex array for a
	   three-stage sky covering several faces. Texture transforms remain per pass. */
	if ( shader->sky.cloudHeight && tess.xstages[0] )
