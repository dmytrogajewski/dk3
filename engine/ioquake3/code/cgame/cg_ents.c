...
#ifdef DK3_GAME
    trap_DK3SoundParams(cent->currentState.number, cent->currentState.dk3SoundVolume,
        cent->currentState.dk3SoundMin, cent->currentState.dk3SoundMax, cent->currentState.dk3SoundFlags);
#endif
...
static void CG_General( centity_t *cent ) {
...
#ifdef DK3_GAME
    if (s1->dk3Effect) DK_DrawWorldEffect(cent);
    if (s1->modelindex && DK_DrawSprite(cent)) return;
#endif
    ...
    ent.backlerp = 0;
#ifdef DK3_GAME
    if (s1->dk3AnimationRate > 0 && s1->dk3AnimationLast >= s1->dk3AnimationFirst) {
        int frames = s1->dk3AnimationLast - s1->dk3AnimationFirst + 1;
        float position = (cg.time - s1->dk3AnimationStart) * s1->dk3AnimationRate / 1000.0f;
        int previous, next;
        if (position < 0) position = 0;
        previous = (int)position;
        ent.backlerp = 1 - (position - previous);
        if (s1->dk3AnimationLoop) { previous %= frames; next = (previous + 1) % frames; }
        else { if (previous >= frames) previous = frames - 1; next = previous + 1 < frames ? previous + 1 : previous; }
        ent.oldframe = s1->dk3AnimationFirst + previous;
        ent.frame = s1->dk3AnimationFirst + next;
        if (s1->dk3RenderFlags & DK3_RF_ANIM_REVERSE) {
            ent.oldframe = s1->dk3AnimationLast - previous;
            ent.frame = s1->dk3AnimationLast - next;
        }
    } else if (cent->interpolate && cent->nextState.modelindex == s1->modelindex) {
        ent.frame = cent->nextState.frame;
        ent.backlerp = 1.0f - cg.frameInterpolation;
    }
#endif
...
#ifdef DK3_GAME
    if (s1->dk3Team == TEAM_RED || s1->dk3Team == TEAM_BLUE) {
        ent.shaderRGBA[0] = s1->dk3Team == TEAM_RED ? 255 : 80;
        ent.shaderRGBA[1] = 80;
        ent.shaderRGBA[2] = s1->dk3Team == TEAM_BLUE ? 255 : 80;
        ent.shaderRGBA[3] = 255;
        trap_R_AddLightToScene(ent.origin, 80, ent.shaderRGBA[0] / 255.0f, 0.15f, ent.shaderRGBA[2] / 255.0f);
    }
#endif
    ...
    {
        int axis;
        float scale = s1->dk3Scale > 0 ? s1->dk3Scale : 1;
        ent.nonNormalizedAxes = qfalse;
        for (axis = 0; axis < 3; ++axis) {
            float amount = s1->dk3ModelScale[axis] > 0 ? s1->dk3ModelScale[axis] : scale;
            VectorScale(ent.axis[axis], amount, ent.axis[axis]);
            if (amount != 1) ent.nonNormalizedAxes = qtrue;
        }
...
static void CG_AddCEntity( centity_t *cent ) {
	...
	case ET_ITEM:
		...
		break;
#ifdef DK3_GAME
    case ET_DK3_MISSILE: DK_DrawProjectile(cent); break;
    case ET_DK3_ITEM: if (!DK_DrawCarriedObjective(cent)) CG_General(cent); break;
    case ET_DK3_EFFECT: DK_DrawWorldEffect(cent); break;
#endif
	case ET_MISSILE:
		CG_Missile( cent );
		break;
