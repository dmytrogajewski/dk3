...
{
#ifdef DK3_GAME
    if (cg.levelShot || !cg_draw2D.integer) return;
    if (cg.snap->ps.dk3CameraActive) { DK_DrawHud(); return; }
    cg.scoreBoardShowing = cg.showScores || cg.snap->ps.pm_type == PM_INTERMISSION || cg.snap->ps.stats[STAT_HEALTH] <= 0;
    if (cg.scoreBoardShowing) DK_DrawScores();
    else {
        DK_DrawHud();
        if (stereoFrame == STEREO_CENTER) CG_DrawCrosshair();
        CG_DrawCrosshairNames();
    }
    DK_DrawCenterMessage();
    return;
#endif
#ifdef MISSIONPACK
