...
static void CG_Obituary( entityState_t *ent ) {
	...
	if (message) {
#ifdef DK3_GAME
        if (attacker == target && mod > DK_MOD_WEAPON_BASE && mod < DK_MOD_WEAPON_BASE + DK_WEAPON_COUNT) {
            CG_Printf("%s died using %s.\n", targetName, dk_weapons[mod - DK_MOD_WEAPON_BASE].label);
            return;
        }
#endif
		CG_Printf( "%s %s.\n", targetName, message);
...
#ifdef DK3_GAME
        if (mod > DK_MOD_WEAPON_BASE && mod < DK_MOD_WEAPON_BASE + DK_WEAPON_COUNT) {
            CG_Printf("%s was killed by %s using %s.\n", targetName, attackerName,
                dk_weapons[mod - DK_MOD_WEAPON_BASE].label);
            return;
        }
#endif
		switch (mod) {
...
void CG_PainEvent( centity_t *cent, int health ) {
	...
	if (CG_WaterLevel(cent) == 3) {
        ...
        trap_S_StartSound(NULL, cent->currentState.number, CHAN_VOICE,
            trap_S_RegisterSound("sounds/hiro/waterchoke1.wav", qfalse));
#else
		if (rand()&1) {
...
#ifdef DK3_GAME
        DK_SubtitleForSound(CG_ConfigString(CS_SOUNDS + es->eventParm));
#endif
...
#ifdef DK3_GAME
    case EV_DK3_BEAM: DK_CombatEffect(cent, qfalse); break;
    case EV_DK3_BLAST: DK_CombatEffect(cent, qtrue); break;
    case EV_DK3_IMPACT: DK_WeaponImpact(cent); break;
#endif
	case EV_DEBUG_LINE:
