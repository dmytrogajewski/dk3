...
void trap_DK3SoundParams(int entity, float volume, float minimum, float maximum, int flags) {
    syscall(CG_DK3_SOUND_PARAMS_V1, entity, PASSFLOAT(volume), PASSFLOAT(minimum), PASSFLOAT(maximum), flags);
}
...
void trap_DK3UpdateSound(void) { syscall(CG_DKQ3_UPDATE_SOUND); }
