...
#ifdef DK3_GAME
    return; /* dk3 draws a bounded damage tint in its HUD pass. */
#endif
	int			t;
	int			maxTime;
	refEntity_t		ent;
...
#ifdef DK3_GAME
    if (ps->dk3CameraActive) {
        float horizontal;
        VectorCopy(ps->dk3CameraOrigin, cg.refdef.vieworg);
        VectorCopy(ps->dk3CameraAngles, cg.refdefViewAngles);
        AnglesToAxis(cg.refdefViewAngles, cg.refdef.viewaxis);
        cg.refdef.fov_x = Com_Clamp(10, 150, ps->dk3CameraFov);
        horizontal = cg.refdef.width / tan(cg.refdef.fov_x * M_PI / 360);
        cg.refdef.fov_y = atan2(cg.refdef.height, horizontal) * 360 / M_PI;
        return 0;
    }
#endif
/*
